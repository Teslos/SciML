...
class SquareDomain:
    ...
    def add_boundary_points(self, n_boundary, random_seed):
        ...
        # every boundary has two points (0 and 1), so we have 2*space_dimensions boundaries
        # to get the boundary points in real space we need to scale the points in the unit square to the real space
        # the scaling is done by the difference between the initial and final values of the domain (extrema_0 and extrema_f)
        # each space dimension is scaled independently.
